...
La primera articulación orienta el manipulador hacia el punto deseado sobre el plano \(XY\).
...
Como el TCP se encuentra separado una distancia \(l_3\) de la articulación de la muñeca, primero se determina la posición del centro de la muñeca.
...
Con estas coordenadas se calcula la distancia entre el hombro y la muñeca.
...
Finalmente se aplica el offset definido por el modelo DH.
...
La última articulación se obtiene imponiendo la orientación deseada del efector final.
...
Cada solución calculada es evaluada mediante la función `structure_clear()`.

Para ello se ejecuta nuevamente la cinemática directa utilizando los ángulos obtenidos y se calcula la posición de:
...
Si existen varias soluciones válidas, se calcula la distancia euclidiana entre cada una de ellas y la configuración articular actual del robot.

Se selecciona la solución cuya distancia sea mínima, reduciendo así el movimiento necesario para alcanzar el objetivo.

### 9. Movimiento del robot

Finalmente se realiza una interpolación lineal entre la configuración actual y la configuración objetivo durante tres segundos.
